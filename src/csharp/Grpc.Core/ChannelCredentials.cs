...
namespace Grpc.Core
{
    ...
    public sealed class SslCredentials : ChannelCredentials
    {
        ...
        private int VerifyPeerCallbackHandler(IntPtr host, IntPtr pem, IntPtr userData, bool isDestroy)
        {
            ...
            {
                // This can be reached when using Mono if it forcibly collects
                // this object when application terminates despite the gcHandle.
                // Logging anything will cause an exception.
                // Return validation error (even though it will be ignored by
                // the native layer).
                return 1;
            }
